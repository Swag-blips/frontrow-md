...
const ReviewResults: React.FC = () => {
      ...
      research: [
        {
          ...
          url: '#',
          excerpt: "This study demonstrates that a solution of 15% L-ascorbic acid, 1% alpha-tocopherol, and 0.5% ferulic acid provides a significant eight-fold increase in photoprotection against solar-simulated radiation."
        }
      ],
      ...
      research: [
        {
          ...
          url: '#',
          excerpt: "Hyaluronic acid's unique capacity to retain water makes it a key component in maintaining skin hydration, elasticity, and volume."
        }
      ],
      ...
      research: [
        {
          ...
          url: '#',
          excerpt: "Maximum absorption was achieved with a 20% vitamin C concentration... an acidic pH (below 3.5) is required for optimal percutaneous absorption."
        }
      ],
  ...
  useEffect(() => {
    const isAnyModalOpen = isModalOpen || isAcceptModalOpen || isRejectModalOpen || 
                          isRejectSuccessModalOpen || isEditModalOpen || isEditSuccessModalOpen;
  ...
  }, [isModalOpen, isAcceptModalOpen, isRejectModalOpen, isRejectSuccessModalOpen, isEditModalOpen, isEditSuccessModalOpen]);
  ...
  return (
    <>
            ...
            <div className="carousel-viewport" ref={carouselViewportRef}>
              <div className="carousel-track" ref={carouselTrackRef}>
                {reviewsData.map((review, index) => (
                  <div
                    key={index}
                    className={`review-card ${index === currentIndex ? 'is-active' : ''}`}
                  ...
                  >
                    ...
                    <p className="review-card__text">{getShortText(review.reviewText)}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="modal is-visible" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="doctor-info">
                <img src={selectedReview.doctorImg} alt={selectedReview.doctorName} />
                <span className="name">{selectedReview.doctorName}</span>
              </div>
              <h2 className="review-card__title">{selectedReview.reviewTitle}</h2>
              <div 
                  ...
                  selectedReview.research.map((item, index) => (
                    <li key={index}>
                      <div className="modal-research-card">
                        <h4 className="modal-research-card__title">{item.title}</h4>
                        <p className="modal-research-card__source">
                          Source: <a href={item.url} target="_blank" rel="noopener noreferrer">{item.source}</a>
                        </p>
                        <p className="modal-research-card__excerpt">{item.excerpt}</p>
                      </div>
                    </li>
                  ))
                ) : (
      ...
      {/* Accept Modal */}
      {isAcceptModalOpen && (
        <div className="modal is-visible" onClick={() => setIsAcceptModalOpen(false)}>
          <div className="modal-content small" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setIsAcceptModalOpen(false)}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" />
              </svg>
            </button>
            <div className="modal-body">
              ...
              <p className="success-message">
                This review has been successfully added to your approved content library.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Reject Modal */}
      {isRejectModalOpen && (
        <div className="modal is-visible" onClick={() => setIsRejectModalOpen(false)}>
          <div className="modal-content small" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <form className="reject-form" onSubmit={handleRejectSubmit}>
                <div className="form-group">
                  <label className="form-label">Why are you rejecting this review? (Optional)</label>
                  <textarea 
      ...
      {isRejectSuccessModalOpen && (
        <div className="modal is-visible" onClick={() => setIsRejectSuccessModalOpen(false)}>
          <div className="modal-content small" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setIsRejectSuccessModalOpen(false)}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" />
              </svg>
            </button>
            <div className="modal-body">
              ...
              <p className="success-message">
                Thank you for your feedback. We'll use this to improve future review generations.
              </p>
            </div>
          </div>
        </div>
      ...
      {isEditModalOpen && selectedReview && (
        <div className="modal is-visible" onClick={() => setIsEditModalOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="current-review-content">
                <strong>{selectedReview.reviewTitle}</strong> ({countWords(selectedReview.reviewText)} words)
                <div dangerouslySetInnerHTML={{ __html: selectedReview.reviewText }} />
              </div>
      ...
      {isEditSuccessModalOpen && updatedReviewData && (
        <div className="modal is-visible" onClick={() => setIsEditSuccessModalOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setIsEditSuccessModalOpen(false)}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" />
              </svg>
            </button>
            <div className="modal-body">
              ...
              <div className="edit-comparison">
                <div className="comparison-section">
                  ...
                  <div className="review-content" dangerouslySetInnerHTML={{ __html: selectedReview?.reviewText || '' }} />
                </div>
                
                <div className="comparison-section">
                  <h3>Updated Review ({countWords(updatedReviewData.reviewText)} words)</h3>
                  <div className="review-content" dangerouslySetInnerHTML={{ __html: updatedReviewData.reviewText }} />
                </div>
              </div>
